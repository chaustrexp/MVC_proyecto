...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    $asignacion = new Asignacion(null, $_POST['instructor_inst_id'], $_POST['asig_fecha_ini'], $_POST['asig_fecha_fin'], $_POST['ficha_fich_id'], $_POST['ambiente_amb_id'], $_POST['competencia_comp_id']);
    ...
    <header class="sticky top-0 z-30 bg-white/80 dark:bg-background-dark/80 backdrop-blur-md border-b border-primary/10">
        <div class="px-5 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold tracking-tight">Nueva Asignación</h1>
                    <p class="text-[10px] uppercase font-bold text-primary tracking-widest">SENA Gestión</p>
                </div>
            </div>
    ...
    <main class="px-5 pt-6 pb-8 max-w-2xl mx-auto">
        <form method="POST" class="space-y-6">
                ...
                <div class="p-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="asig_fecha_ini" required
                        ...
                        <div>
                            ...
                            <input type="date" name="asig_fecha_fin" required
                                class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-secondary-green focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <select name="ficha_fich_id" required
                            ...
                            <option value="<?= $ficha->getFichId() ?>"><?= $ficha->getFichId() ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="ambiente_amb_id" required
                            ...
                            <option value="<?= $ambiente->getAmbId() ?>"><?= $ambiente->getAmbNombre() ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="competencia_comp_id" required
                            ...
                            <option value="<?= $competencia->getCompId() ?>"><?= $competencia->getCompNombreCorto() ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="flex gap-3">
                <button type="submit" 
                    class="flex-1 bg-primary text-white py-4 rounded-xl font-bold text-base hover:bg-primary/90 transition-colors shadow-lg shadow-primary/30 flex items-center justify-center gap-2">
