...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    $detalle->setAsigId($_POST['asig_id']);
    $detalle->setDetasigHoraIni($_POST['detasig_hora_ini']);
    $detalle->setDetasigHoraFin($_POST['detasig_hora_fin']);
    AsignacionDetalle::update($detalle);
...
<head>
    ...
    <title>Editar Sesión - SENA</title>
    ...
    <header class="sticky top-0 z-50 bg-primary text-white shadow-md">
        <div class="px-4 py-3 flex items-center justify-between">
            ...
            <div class="text-center flex-1 mx-2">
                <h1 class="text-lg font-bold leading-tight">Editar Sesión</h1>
                <p class="text-[10px] uppercase tracking-wider opacity-90 font-medium">ID: <?= $detalle->getDetasigId() ?></p>
            </div>
            ...
            <section class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-emerald-100 dark:border-emerald-900/30 overflow-hidden">
                ...
                <div class="p-4">
                    ...
                    <select name="asig_id" required
                        ...
                        <option value="<?= $asignacion->getAsigId() ?>" <?= $asignacion->getAsigId() == $detalle->getAsigId() ? 'selected' : '' ?>>
            ...
            <section class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-emerald-100 dark:border-emerald-900/30 overflow-hidden">
                ...
                <div class="p-4 space-y-4">
                    <div>
                        ...
                        <input type="time" name="detasig_hora_ini" value="<?= $detalle->getDetasigHoraIni() ?>" required
                    ...
                    <div>
                        ...
                        <input type="time" name="detasig_hora_fin" value="<?= $detalle->getDetasigHoraFin() ?>" required
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm font-medium">
                    </div>
                    
                    <!-- Time Preview -->
                    <div class="pt-3 border-t border-emerald-50 dark:border-emerald-900/20">
                        <p class="text-[10px] text-emerald-600 dark:text-emerald-500 font-bold uppercase mb-2">Horario Actual</p>
                        <div class="flex items-center gap-2">
                            <span class="bg-primary/10 text-primary px-3 py-1 rounded text-sm font-bold tracking-tight"><?= date('h:i A', strtotime($detalle->getDetasigHoraIni())) ?></span>
                            <span class="material-icons text-slate-300 text-sm">arrow_forward</span>
                            <span class="bg-primary/10 text-primary px-3 py-1 rounded text-sm font-bold tracking-tight"><?= date('h:i A', strtotime($detalle->getDetasigHoraFin())) ?></span>
                        </div>
                    </div>
                </div>
            </section>
