...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    $programa->setProgDenominacion($_POST['prog_denominacion']);
    $programa->setTitproId($_POST['titpro_id']);
    $programa->setProgTipo($_POST['prog_tipo']);
...
<header class="bg-primary sticky top-0 z-50 shadow-md">
    <div class="px-6 py-4 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-white text-xl font-bold leading-tight">Editar Programa</h1>
            <p class="text-white/80 text-xs font-light">Modificar información</p>
        </div>
    </div>
</header>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-100 dark:border-slate-800">
            <div class="flex items-center gap-3 mb-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                <div class="bg-primary/10 p-3 rounded-xl">
                    <span class="material-icons text-primary text-2xl">school</span>
                </div>
                <div>
                    <h2 class="text-xs font-bold text-primary-dark uppercase tracking-wider">Código del Programa</h2>
                    <p class="text-lg font-bold text-slate-800 dark:text-white">#<?= $programa->getProgCodigo() ?></p>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input 
                        ...
                        value="<?= $programa->getProgDenominacion() ?>"
                        required
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="<?= $titulo->getTitproId() ?>" <?= $titulo->getTitproId() == $programa->getTitproId() ? 'selected' : '' ?>>
