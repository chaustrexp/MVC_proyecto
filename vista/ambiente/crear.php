...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    $ambiente = new Ambiente(null, $_POST['amb_nombre'], $_POST['sede_id']);
...
<header class="bg-primary text-white sticky top-12 z-40 px-5 py-4 shadow-md rounded-b-xl">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-xl font-bold tracking-tight">Crear Nuevo Ambiente</h1>
    </div>
</header>
...
<main class="mt-6 px-5 pb-32">
    <form method="POST" class="space-y-6">
        <div class="bg-white dark:bg-background-dark/50 border border-primary/5 dark:border-primary/20 rounded-xl p-5 shadow-sm">
            <h2 class="text-xs font-semibold text-primary/70 dark:text-primary uppercase tracking-wider mb-4">Información del Ambiente</h2>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="<?= $sede->getSedeId() ?>"><?= $sede->getSedeNombre() ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
